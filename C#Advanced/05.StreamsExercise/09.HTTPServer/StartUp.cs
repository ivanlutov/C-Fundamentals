...
namespace _09.HTTPServer
{
    ...
    {
        private const int PortNumber = 1331;
                    ...
                    try
                    {
                        ...
                        using (sw)
                        {
                            ...
                            else if (page == "/info")
                            {
                                ...
                                msg =
                                    $"<!doctype html> <html> <head> <title>Info Page</title> </head> <body> <h2>Current Time: {DateTime.Now.ToString("dd MMM yyyy HH:mm:ss", ci)}</h2> <h2>Logical Processors: {Environment.ProcessorCount}</h2><h4>Back to the <a href=\"/\">homepage</a></h4> </body> </html>";
                                sw.Write(msg);
                            }
                            else
                            {
